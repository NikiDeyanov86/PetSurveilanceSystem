    ...
    while hl.knock() == "Knock Recieved":
        ...
        if hl.learnedBlocks() is not None:
            ...
            if (target.x - (target.width / 2)) < leftOffset:
                ...
                div = calculate_div(diff)
                mqttClient.publish(topic_publish, "left,{sec},{speed}".format(sec=diff / div, speed=motorSpeed+15))
                time.sleep(diff / div)

            elif (target.x + (target.width / 2)) > rightOffset:
                diff = (target.x + (target.width / 2)) - rightOffset
                div = calculate_div(diff)
                mqttClient.publish(topic_publish, "right,{sec},{speed}".format(sec=diff / div, speed=motorSpeed+15))
                time.sleep(diff / div)
